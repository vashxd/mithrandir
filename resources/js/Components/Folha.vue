...
<template>
    <Teleport to="body">
        ...
        >
            <div
                v-if="aberta"
                class="fixed inset-0 z-[55] hidden bg-veu/45 lg:block"
                @click="emit('fechar')"
            />
        </Transition>
        ...
        >
            <div
                v-if="aberta"
                class="fixed inset-0 z-[60] flex flex-col bg-superficie-2
                       ...
                       lg:border lg:border-borda lg:bg-superficie lg:shadow-2xl"
            >
                <header class="flex items-center gap-3 border-b border-borda bg-superficie px-4 py-3 pt-safe lg:pt-3">
                    <button
                        type="button"
                        class="-ml-2 flex h-11 w-11 items-center justify-center rounded-full text-tinta-2 transition active:bg-superficie-2 lg:order-3 lg:-ml-0 lg:-mr-2 lg:h-9 lg:w-9 lg:hover:bg-superficie-2"
                        aria-label="Fechar"
                        @click="emit('fechar')"
                    >
                        <Icone nome="x" class="h-6 w-6 lg:h-5 lg:w-5" />
                    </button>
                    <h2 class="flex-1 truncate text-lg font-bold text-tinta lg:order-1">{{ titulo }}</h2>
                ...
                <div
                    v-if="$slots.rodape"
                    class="border-t border-borda bg-superficie px-4 py-3 pb-safe lg:px-6 lg:pb-3"
                >
                    <div class="mx-auto max-w-2xl">
                        <slot name="rodape" />
                    </div>
